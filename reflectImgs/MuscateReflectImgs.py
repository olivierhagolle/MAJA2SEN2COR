...
"""
Class for reflectance Muscate product
...
class MuscateReflectImgs:
    ...
    def Convert( cls
        ...
        for root,dirnames,filenames in os.walk(_s_productPath):
            for filename in fnmatch.filter(filenames, '*_'+_s_reflType+'_*tif'):
        ...
        for i in range(len(t_matches)):
            ...
            # band name
            s_band = t_matches[i].split('.')[0].split('_')[-1]
            if len(s_band)==2:
                s_band = s_band[0] + '0' + s_band[1]
        
            # Create reflectance image name
